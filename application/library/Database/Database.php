 ...
 * [Database_Src_Query] and [Database_Src_Query_Builder] objects, which can be easily
...
abstract class Database {
	...
	public static function instance($name = NULL, array $config = NULL)
	{
		...
		if ( ! isset(Database::$instances[$name]))
		{
			...
			{
				throw new Yaf_Exception('Database type not defined in :name configuration');
			}

			// Set the driver class name
			$driver = 'Database\Classes\\'.ucfirst($config['type']);
	 ...
	 * @return  object   Database_Src_Result for SELECT queries
	 ...
	 *          $db->commit();
	 *     }
	 *     catch (Database_Src_Exception $e)
	 *     {
	 ...
	 * [Database_Src_Expression] objects will be compiled.
	 * [Database_Src_Query] objects will be compiled and converted to a sub-query.
	...
	public function quote($value)
	{
		...
		{
			if ($value instanceof Database_Src_Query)
			{
				// Create a sub-query
				return '('.$value->compile($this).')';
			}
			elseif ($value instanceof Database_Src_Expression)
			{
	...
	public function quote_column($column)
	{
		...
		if ($column instanceof Database_Src_Query)
		{
			// Create a sub-query
			$column = '('.$column->compile($this).')';
		}
		elseif ($column instanceof Database_Src_Expression)
		{
			// Compile the expression
			$column = $column->compile($this);
		}
	...
	public function quote_table($table)
	{
		...
		if ($table instanceof Database_Src_Query)
		{
			// Create a sub-query
			$table = '('.$table->compile($this).')';
		}
		elseif ($table instanceof Database_Src_Expression)
		{
			// Compile the expression
			$table = $table->compile($this);
		}
